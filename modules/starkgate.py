...
class Starkgate:
    ...
    def __init__(self, key, number, params):
        ...
        self.starknet_address = STARKNET_WALLETS[self.key] # ПРОВЕРИТЬ
    ...
    async def get_txn(self):
        ...
        try:
            contract = self.manager.web3.eth.contract(address=Web3.to_checksum_address("0xae0Ee0A63A2cE6BaeEFFE56e7714FB4EFE48D419"), abi=ABI_STARKGATE)
            gas_l2 = await self.get_l2_gas(self.amount)

            contract_txn = await contract.functions.deposit(
                    self.amount,
                    int(self.starknet_address, 16)
                ).build_transaction(
                {
                    "from": self.manager.address,
                    "value": int(self.amount + gas_l2),
